  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          aws_codebuild_project.rhobs_apply.arn,
          aws_codebuild_project.rhobs_bootstrap.arn
        ]
      }
    ]
  })
...
resource "aws_codebuild_project" "rhobs_apply" {
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = "terraform/config/pipeline-rhobs-cluster/buildspec-provision-infra.yml"
  }
}

# CodeBuild Project - Bootstrap ArgoCD
resource "aws_codebuild_project" "rhobs_bootstrap" {
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = "terraform/config/pipeline-rhobs-cluster/buildspec-bootstrap-argocd.yml"
  }
}
...
resource "aws_codepipeline" "central_pipeline" {
  ...
  trigger {
    ...
    git_configuration {
      ...
      push {
        ...
        file_paths {
          includes = ["deploy/${var.target_environment}/${var.target_region}/pipeline-rhobs-cluster-inputs/terraform.json", "terraform/config/pipeline-rhobs-cluster/**"]
        }
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = aws_codebuild_project.rhobs_apply.name
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = aws_codebuild_project.rhobs_bootstrap.name
        EnvironmentVariables = jsonencode([
          {
            name  = "IS_DESTROY"
            value = "#{variables.IS_DESTROY}"
